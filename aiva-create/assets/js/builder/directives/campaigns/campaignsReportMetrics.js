...
angular.module('builder').directive('campaignsReportMetrics', ['$rootScope', function ($rootScope ) {
    return {
        restrict: 'A',
        scope: {
            list: '=',
        ...
        templateUrl: 'views/campaigns/report-metrics.html',
...
/*            
            $scope.select = function(val) {
                $scope.ngModel = val;
                $scope.dropdownOpen = false;
                $rootScope.$broadcast('metrics.changed', val);
                $element.focus();
            };
...
*/
            $scope.toggle = function() {
                ...
                if (!nextState) $rootScope.$broadcast('dropdowns.hide', 'metrics');
                $scope.dropdownOpen = nextState;
            };
            $scope.open = function() {
                $rootScope.$broadcast('dropdowns.hide', 'metrics');
