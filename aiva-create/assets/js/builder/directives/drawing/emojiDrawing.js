(function() {
    ...
    angular.module('builder').directive('aivaEmojiDrawing', aivaEmojiDrawing);
    
    function aivaEmojiDrawing() {
        var directive = {
            restrict: 'A',
            link: emojiDrawingLink,
            controller: EmojiDrawingController,
            controllerAs: 'vm'
        };
        
        return directive;
            
        function emojiDrawingLink($scope, el, attrs, ctrl) {
    ...
    EmojiDrawingController.$inject = 
    ['$scope', '$rootScope', 'elementDrawing', 'aivaElementConfig', '$compile', 'elementScaling', 'cssUpdates', 'ctaBase', 'undoRedo', 'aivaVariant', 'draggable'];
    
    
    function EmojiDrawingController($scope, $rootScope, elementDrawing, aivaElementConfig, $compile, elementScaling, cssUpdates, ctaBase, undoRedo, aivaVariant, draggable) {
        var vm = this;
        vm.coordStart = {};
        vm.pageCoordStart = {};
        vm.coordLatest = {};
        ...
        vm.isActivated = function() {
            return elementDrawing.getActiveDrawingElement() === 'emoji';
        };
        ...
        vm.beginDrawing = function(e) {
            ...
            vm.pageOffsetDiff = {x: vm.pageCoordStart.x - vm.coordStart.x, y: vm.pageCoordStart.y - vm.pageCoordStart.y};
            vm.isDrawing = true;
        };
        ...
        vm.updateDrawing = function(e) {
            ...
            if (checkedValues.width > 10 && checkedValues.height > 10) {
                ...
                if (!vm.drawn) {
                    var emojiSrc = 'assets/images/emojis/512/' + elementDrawing.activeDrawingElementClass + '.png';
                    var template = format(elementConfig.template, emojiSrc, elementDrawing.activeDrawingElementId, elementDrawing.activeDrawingElementClass);
