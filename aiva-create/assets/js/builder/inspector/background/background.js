angular.module('builder.inspector')
    ...
    .controller('BackgroundController', ['$scope', 'inspector', 'classHelper', '$rootScope', function ($scope, inspector, classHelper, $rootScope) {
        ...
        //         // console.log('properties watchcollection', newProps);
        //         for (var prop in newProps) {
        //             if (prop === 'color' && newProps[prop]) { //todo: look into problem of colors coming in rgb and hex. make consistently one or the other
        //                 var newColor = tinycolor(newProps[prop]);
        //                 newProps[prop] = newColor.toRgbString();
        //                 var oldColor = tinycolor(oldProps[prop]);
        //                 oldProps[prop] = oldColor.toRgbString();
        //             }
        ...
        //                 var style = prop === 'color' ? 'color' : 'background';
        //                 // if (iconForegroundColoringMode) style = 'color';
        //                 var classes = classHelper.splitClasses($rootScope.selected.node);


        //                 _.forEach(classes, function (aClass) {
        //                     if (aClass.indexOf('triangle-up') > -1) {
        //                         style = 'border-bottom-color';
        //                     } else if (aClass.indexOf('triangle-down') > -1) {
        //                         style = 'border-top-color';
        //                     } else if (aClass.indexOf('triangle-left') > -1) {
        //                         style = 'border-right-color';
        //                     } else if (aClass.indexOf('triangle-right') > -1) {
        //                         style = 'border-left-color';
        //                     } else if (aClass.indexOf('triangle-top-left') > -1) {
        //                         style = 'border-left-color';
        //                     } else if (aClass.indexOf('triangle-top-right') > -1) {
        //                         style = 'border-right-color';
        //                     } else if (aClass.indexOf('triangle-bottom-left') > -1) {
        //                         style = 'border-left-color';
        //                     } else if (aClass.indexOf('triangle-bottom-right') > -1) {
        //                         style = 'border-right-color';
        //                     }
        //                 });
        ...
        $scope.$on('element.reselected', function (e, node) {
            ...
            console.warn('element.reselected', $scope.selected);
            $scope.properties.color = $scope.selected.getStyle('background-color');
            $rootScope.colors.background = $scope.properties.color;
            $rootScope.colors.foreground = $scope.selected.getStyle('color');
        });

        $scope.$on('element.deselected', function (e, node) {
            $scope.properties.color = null;
            $rootScope.colors.background = null;
            $rootScope.colors.foreground = null;
        });
        ...
        $scope.$on('properties.color.slidestart', function (e) {
            ...
            console.log('on properties.color.slidestart');
        ...
        $scope.$on('properties.color.slidestop', function (e) {
            console.log('on properties.color.slideStop');
            $scope.command.newStyles = $scope.properties.color;
        });

        $scope.open = function() {
            $rootScope.$broadcast('openColorTab', 'background');
        };

        $scope.$on('color.change', function(e, data) {
            if (data[0] === 'foreground') {
                inspector.applyCss( 'color', data[1], {});
                // console.info('foreground.color.change', $scope.selected, data[1]);
                $scope.properties.color = data[1];
            } else if (data[0] === 'background') {
                inspector.applyCss( 'background', data[1], {});
                // console.info('background.color.change', $scope.selected, data[1]);
                $scope.properties.backgroundColor = data[1];
            }            
        });
    ...
    .directive('blImgAlignmentGrid', ['$compile', function ($compile) {
        return {
            restrict: 'A',
            template: '<div ng-class="{active: properties.position == \'top left\'}" ng-click="changeAlignment(\'top left\')" class="alignment-box"></div>' +
